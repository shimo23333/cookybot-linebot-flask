...
class RecipeGenerator:
    ...
    def generate_dinner_image(self, recipe_text):
        ...
        try:
            print("生成食物圖片中...")
            style=f"A photorealistic close-up of freshly prepared food: {recipe_text}, showcasing vivid textures, realistic lighting, soft natural shadows, and subtle moisture or steam if applicable. The food is arranged naturally on a [描述擺放表面，例如 rustic wooden board, ceramic plate] in a well-lit environment with natural sunlight streaming through. The image features DSLR-quality, sharp focus, and realistic proportions. Negative Prompt: cartoon, 2D, illustration, painting, CGI, 3D render, plastic texture, unrealistic colors, over-saturated, blurry, flat lighting, artificial reflections, bad proportions, fake-looking food, low detail, low resolution, pixelated textures, smooth plastic-like surfaces, over-polished, distorted features, extra limbs, floating items, unnatural highlights, synthetic materials, overexposed, underexposed, color grading, distracting background, heavy filters, glowing objects."
            # style=(
